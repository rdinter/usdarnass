...
If you are not comfortable with either of these options, then to use the package you need to ensure that the `token` parameter is set to your API key in each of your function calls that queries USDA Quickstats.
...
The first argument in `nass_param()` is the parameter of interest, which can take on any of the 18 values from the parameters section. This argument must be passed in a character format, so make sure to use quotations in your calls.

This function is most helpful in determining what variables are available in for a certain subset. For example, if I were interested in what county level variables in Ohio are available in 2000 I might start by determining what "Group" is available at that level:

```{r ohio_group}
nass_param("group_desc", state_name = "OHIO", year = 2000)
```

I should mention that the `year` parameter is the only parameter that does not need to be a character vector but can be numeric. Now, if I want to further figure out what commodities are available for the "DAIRY" subset of this data, I would make a call of :

```{r ohio_commodity}
nass_param("commodity_desc", group_desc = "dairy", state_name = "OHIO", year = 2000)
```

One point of emphasis with the above code is that the character vectors are not case sensitive for the parameters and 
